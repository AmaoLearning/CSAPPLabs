...
int isAsciiDigit(int x) {
  return !((~(((x + (~0x30 + 1)) >> 31) ^ ((x + (~0x3a + 1)) >> 31)))) & !(x >> 31);
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  int case2 = ((signx & signy) | (~signx & ~signy)) & !((y + ~x + 1) >> 31); 
  return case1 | case2;
}
//4
/* 
...
unsigned float_twice(unsigned uf) {
  ...
  unsigned frac = uf & ~(((255 << 1) + 1) << 23);
  if (!(exp ^ 255)) return uf;
  if (!exp) return sign | (uf << 1);
  if (!(exp ^ 254)) return sign | (255 << 23);
...
int float_f2i(unsigned uf) {
  int sign = uf >> 31;
  int exp = (uf >> 23) & 255;
  int frac = ((uf & ~(((255 << 1) + 1) << 23)) + (1 << 23)) << 7;
  if (!(((exp + ~157 + 1) >> 31))) return 0x80000000u;
  exp = exp + ~127 + 1;
  if (!((exp >> 31) + 1)) return 0;
  if (sign) return ~(frac >> (30 - exp)) + 1;
  return frac >> (30 - exp);
}
